...
import { RejectRequestDialogComponent } from './../../Dialogs/reject-request-dialog/reject-request-dialog.component';


@Component({
  selector: 'app-get-employee-request',
  templateUrl: './get-employee-request.component.html',
  styleUrls: ['./get-employee-request.component.scss']
})
export class GetEmployeeRequestComponent implements OnInit {
  visible = true;
  selectable = true;
  removable = true;
  addOnBlur = true;
  ...
  constructor(
    ...
    ngOnInit() {
    	this.updateupdateEmployeeStatus();
  ...
  this.requestsForm = this.fb.group({
   ...
   updateupdateEmployeeStatus(){
    var id = this.actRoute.snapshot.paramMap.get('id');
      this.employeeAPI.updateEmployeeStatus(id).subscribe(res => {
      });
  }
  ...
  updateUser() {
    ...
    if (window.confirm('Are you sure you want to update?')) {
      ...
      this.employeeAPI.updateEmployeeRole(id, this.requestsForm.value).subscribe( res => {
  ...
  approveEmployee() {
    var id = this.actRoute.snapshot.paramMap.get('id')
      this.employeeAPI.UpdateEmployee(id, this.approvalForm.value).subscribe();
  }
